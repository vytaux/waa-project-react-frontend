...
function AdminDashboard({ currentUser }) {
    ...
    const refreshOwners= () => {
        FetchService.getPendingOwners(currentUser.accessToken)
            .then(response => setOwnersState(response.data));
    }
    ...
    const approveOwner = (userId) => {
        FetchService.approveOwner(currentUser.accessToken, userId)
            .then(() => refreshOwners());
    }
